...
O runner Windows executa elevado; o WebView2 ignora overrides de ambiente nesse contexto. Por isso, o teste usa uma política HKLM temporária, restrita a `silonr-desktop.exe` e à porta de loopback, e a remove em `finally`. Esse script é exclusivo do CI e não é incluído no instalador. Consulte a [documentação de segurança do WebView2](https://learn.microsoft.com/en-us/microsoft-edge/webview2/concepts/security#for-an-elevated-host-app-use-appropriate-override-flags). No PC do cliente, abra o Desktop normalmente, sem executar como administrador.
