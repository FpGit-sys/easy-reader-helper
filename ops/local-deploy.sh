...
wait_for_url() {
  ...
  for _ in $(seq 1 30); do
    if curl --noproxy '*' --fail --silent --show-error --max-time 5 --cacert "$ca" --resolve "$SILONR_LOCAL_SERVER:443:$SILONR_LOCAL_BIND_IP" --resolve "$SILONR_LOCAL_SERVER:9443:$SILONR_LOCAL_BIND_IP" "$url" >/dev/null; then
      return 0
    fi
    sleep 1
  done
