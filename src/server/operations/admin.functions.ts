...
function validateRoleScope(role: z.infer<typeof memberRoleSchema>, facilityId: string | null) {
  ...
  if ((role === "gestor_unidade" || role === "inspetor") && !facilityId) throw new Error("ROLE_REQUIRES_FACILITY");
}

async function assertFacilityBelongsToOrganization(organizationId: string, facilityId: string | null) {
  if (!facilityId) return;
  const [facility] = await getDb().select({ id: facilities.id }).from(facilities).where(and(eq(facilities.id, facilityId), eq(facilities.organizationId, organizationId), eq(facilities.active, true))).limit(1);
  if (!facility) throw new Error("INVALID_FACILITY_SCOPE");
}

async function getCurrentLicense(organizationId: string) {
  const [license] = await getDb().select().from(licenses).where(eq(licenses.organizationId, organizationId)).orderBy(desc(licenses.createdAt)).limit(1);
  return license ?? null;
}

async function countActiveUsers(organizationId: string) {
  const rows = await getDb().select({ userId: memberships.userId }).from(memberships).where(and(eq(memberships.organizationId, organizationId), eq(memberships.active, true)));
...
async function assertUserLimit(organizationId: string, userId?: string) {
  ...
  if (!license) return;
  if (userId) {
    const existing = await getDb().select({ id: memberships.id }).from(memberships).where(and(eq(memberships.organizationId, organizationId), eq(memberships.userId, userId), eq(memberships.active, true))).limit(1);
    if (existing.length > 0) return;
  }
  if ((await countActiveUsers(organizationId)) >= license.maxUsers) throw new Error("LICENSE_USER_LIMIT_REACHED");
}

async function assertFacilityLimit(organizationId: string) {
  const license = await getCurrentLicense(organizationId);
  if (!license) return;
  const rows = await getDb().select({ id: facilities.id }).from(facilities).where(and(eq(facilities.organizationId, organizationId), eq(facilities.active, true)));
  if (rows.length >= license.maxFacilities) throw new Error("LICENSE_FACILITY_LIMIT_REACHED");
}

export const getProductionAdministration = createServerFn({ method: "GET" }).validator(organizationScopeSchema).handler(async ({ data }) => {
...
export const updateProductionOrganization = createServerFn({ method: "POST" }).validator(updateOrganizationSchema).handler(async ({ data }) => {
  ...
  const [after] = await db.update(organizations).set({ name: data.name, legalName: data.legalName || null, document: data.document || null, updatedAt: new Date() }).where(eq(organizations.id, data.organizationId)).returning();
  if (!after) throw new Error("ORGANIZATION_UPDATE_FAILED");
  await writeAuditEvent({ organizationId: data.organizationId, actorUserId: session.user.id, eventType: "organization.updated", entityType: "organization", entityId: data.organizationId, before: { name: before.name, legalName: before.legalName, document: before.document }, after: { name: after.name, legalName: after.legalName, document: after.document } });
  return { ok: true };
});
...
export const createProductionFacility = createServerFn({ method: "POST" }).validator(createFacilitySchema).handler(async ({ data }) => {
  ...
  await writeAuditEvent({ organizationId: data.organizationId, facilityId: created.id, actorUserId: session.user.id, eventType: "facility.created", entityType: "facility", entityId: created.id, after: { name: created.name, city: created.city, state: created.state } });
...
export const updateProductionFacility = createServerFn({ method: "POST" }).validator(updateFacilitySchema).handler(async ({ data }) => {
  ...
  const [after] = await db.update(facilities).set({ name: data.facility.name, city: data.facility.city || null, state: data.facility.state || null, updatedAt: new Date() }).where(eq(facilities.id, before.id)).returning();
  if (!after) throw new Error("FACILITY_UPDATE_FAILED");
  await writeAuditEvent({ organizationId: data.organizationId, facilityId: after.id, actorUserId: session.user.id, eventType: "facility.updated", entityType: "facility", entityId: after.id, before: { name: before.name, city: before.city, state: before.state }, after: { name: after.name, city: after.city, state: after.state } });
  return { ok: true };
});
...
export const archiveProductionFacility = createServerFn({ method: "POST" }).validator(archiveFacilitySchema).handler(async ({ data }) => {
  ...
  const [target] = await db.select().from(facilities).where(and(eq(facilities.id, data.facilityId), eq(facilities.organizationId, data.organizationId), eq(facilities.active, true))).limit(1);
  if (!target) throw new Error("NOT_FOUND:FACILITY");
  const directMembers = await db.select({ id: memberships.id }).from(memberships).where(and(eq(memberships.organizationId, data.organizationId), eq(memberships.facilityId, data.facilityId), eq(memberships.active, true))).limit(1);
  if (directMembers.length > 0) throw new Error("FACILITY_HAS_ACTIVE_MEMBERS");
  await db.update(facilities).set({ active: false, updatedAt: new Date() }).where(eq(facilities.id, target.id));
  await writeAuditEvent({ organizationId: data.organizationId, facilityId: target.id, actorUserId: session.user.id, eventType: "facility.archived", entityType: "facility", entityId: target.id, before: { active: true, name: target.name }, after: { active: false, name: target.name } });
  return { ok: true };
});
...
export const createProductionMember = createServerFn({ method: "POST" }).validator(createMemberSchema).handler(async ({ data }) => {
  ...
  const pool = getPool();
  const existingAuthResult = await pool.query<{ id: string; email: string }>('select id, email from "user" where lower(email) = lower($1) limit 1', [data.email]);
  let userId = existingAuthResult.rows[0]?.id ?? null;
  if (userId) {
    const assigned = await getDb().select({ id: memberships.id }).from(memberships).where(eq(memberships.userId, userId)).limit(1);
    if (assigned.length > 0) throw new Error("USER_ACCOUNT_ALREADY_ASSIGNED");
  } else {
    await assertUserLimit(data.organizationId);
    await getAuth().api.createUser({ body: { email: data.email, password: data.temporaryPassword, name: data.name, role: "user" } });
    const createdAuthResult = await pool.query<{ id: string }>('select id from "user" where lower(email) = lower($1) limit 1', [data.email]);
  ...
  const duplicate = await getDb().select({ id: memberships.id }).from(memberships).where(and(eq(memberships.organizationId, data.organizationId), eq(memberships.userId, userId), data.facilityId ? eq(memberships.facilityId, data.facilityId) : isNull(memberships.facilityId))).limit(1);
  if (duplicate.length > 0) throw new Error("MEMBERSHIP_ALREADY_EXISTS");
  const [membership] = await getDb().insert(memberships).values({ organizationId: data.organizationId, facilityId: data.facilityId, userId, role: data.role, active: true }).returning({ id: memberships.id });
  if (!membership) throw new Error("MEMBERSHIP_CREATE_FAILED");
  await writeAuditEvent({ organizationId: data.organizationId, facilityId: data.facilityId, actorUserId: session.user.id, eventType: "membership.created", entityType: "membership", entityId: membership.id, after: { userId, email: data.email, role: data.role, facilityId: data.facilityId, active: true }, metadata: { temporaryPasswordStored: false } });
...
export const updateProductionMember = createServerFn({ method: "POST" }).validator(updateMemberSchema).handler(async ({ data }) => {
  ...
  await assertFacilityBelongsToOrganization(data.organizationId, data.facilityId);
  const db = getDb();
  const [before] = await db.select().from(memberships).where(and(eq(memberships.id, data.membershipId), eq(memberships.organizationId, data.organizationId))).limit(1);
  if (!before) throw new Error("NOT_FOUND:MEMBERSHIP");
  if (before.role === "super_admin" && scope.role !== "super_admin") throw new Error("FORBIDDEN:SUPER_ADMIN_MEMBERSHIP");
  if (before.userId === session.user.id && (!data.active || data.role !== "admin_empresa" || data.facilityId !== null)) throw new Error("SELF_ADMIN_MEMBERSHIP_PROTECTED");
  ...
  if (before.active && before.role === "admin_empresa" && before.facilityId === null) {
    const otherAdmins = await db.select({ id: memberships.id }).from(memberships).where(and(eq(memberships.organizationId, data.organizationId), eq(memberships.active, true), eq(memberships.role, "admin_empresa"), isNull(memberships.facilityId), ne(memberships.id, before.id)));
    if (otherAdmins.length === 0 && (!data.active || data.role !== "admin_empresa" || data.facilityId !== null)) throw new Error("LAST_ORGANIZATION_ADMIN_REQUIRED");
  }
  const [after] = await db.update(memberships).set({ role: data.role, facilityId: data.facilityId, active: data.active, updatedAt: new Date() }).where(eq(memberships.id, before.id)).returning();
  if (!after) throw new Error("MEMBERSHIP_UPDATE_FAILED");
  await writeAuditEvent({ organizationId: data.organizationId, facilityId: data.facilityId ?? before.facilityId, actorUserId: session.user.id, eventType: "membership.updated", entityType: "membership", entityId: after.id, before: { userId: before.userId, role: before.role, facilityId: before.facilityId, active: before.active }, after: { userId: after.userId, role: after.role, facilityId: after.facilityId, active: after.active } });
  return { ok: true };
});
