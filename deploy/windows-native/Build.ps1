...
foreach ($dir in @('app','runtime','tools','config','migrations','postgres','caddy','desktop','services','licenses')) { New-Item -ItemType Directory -Path (Join-Path $stage $dir) | Out-Null }
...
Expand-Archive $pg (Join-Path $stage 'postgres')
